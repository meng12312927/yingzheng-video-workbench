...
- 素材优先的需求分析：系统先转录并由代码检索证据，再用一次 LLM 调用提出最多 5 个业务问题；问题、用途、受众、风格、字幕、音乐和内容要求在同一页确认，不需要逐题重复提交。
- BM25 + Embedding/RRF 混合检索。BM25 负责姓名、奖项、产品名等精确词，向量检索补充语义近似表达；素材访谈与候选阶段共享同一素材向量索引。
- 证据化候选：代码先为全部要求检索，候选排序器只调用一次并返回紧凑选择；引用、需求关联、来源和分数由代码补齐。模型失败或少选时，可验证的检索候选会带风险提示进入人工审核，不再导致整份方案不可出片。
- 确定性时长预算、必须项覆盖、候选多样性排序、问答上下文保护和短版人工例外。页面会说明当前还差多少秒以及应补哪类内容。
- 电脑审核页按需生成带前后文的候选预览，可保留或删除、排序、修改字幕与标题；拖动边界会定位视频并显示附近原话，也可将当前画面设为剪辑点。时长不足时直接使用“自动补选”或“自己补入片段”，不必寻找隐藏入口。
...
- 状态、任务书、计划、用户决定、模型调用元数据、重试原因和验收报告均版本化并可恢复。相同素材的 Whisper 转录按内容哈希缓存，Embedding 矩阵在阶段间复用。
...
浏览器访问终端显示的地址。推荐流程：上传独立素材并写一句目标 → 查看素材摘要与 AI 问题 → 一次确认任务书 → 审核候选和时间线 → 一次确认生成或交接 → 查看逐项验收 → 下载剪映交接包或 OTIO。
...
| 变量 | 默认值 | 用途 |
| --- | --- | --- |
| `LLM_API_KEY` | 无 | 素材访谈、一次候选批量排序和可选语义校对 |
